  ...
  const handleJoinMeeting = async (roomName) => {
    try {
      if (roomName.trim()) {
        ...
        let roomExists = await axios.get(
          `${server}/api/v1/users/searchMeetings/${roomName}`,
        );
        if (roomExists.success) {
          await axios.post(
            `${server}/api/v1/users/add_activity/${username}`,
            meetingDetails,
          ); // this adds the meeting to the db as soon as the user joins
